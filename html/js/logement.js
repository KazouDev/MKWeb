    ...
    if (
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          document.getElementById('prix__total').innerHTML = JSON.parse(xhr.responseText).prix_ht;
          document.getElementById('prix__TTC').innerHTML = JSON.parse(xhr.responseText).base_tarif;
          
          document.getElementById('nb_jours').innerHTML = JSON.parse(xhr.responseText).nombre_jour;
          document.getElementById('frais__total').innerHTML = JSON.parse(xhr.responseText).frais;
          document.getElementById('taxes__total').innerHTML = JSON.parse(xhr.responseText).taxe;
          document.getElementById('tot-ttc').innerHTML = JSON.parse(xhr.responseText).prix_ttc;
...
const afficherCalendrier = (mois, annee) => {
    ...
    for (var semaine = 1; semaine < 6; semaine++) {
        ligne = table.insertRow();
        for (var jour = 0; jour < 7 && jourCourant <= nombreDeJoursDansMois; jour++) {
            var cellule = ligne.insertCell();
            cellule.innerHTML = jourCourant;
